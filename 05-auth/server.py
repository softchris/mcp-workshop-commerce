...
# from mcp.server.request import RequestContext
...
# Request-scoped storage (better than globals)
...
class CustomHeaderMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        # Store token in context var (request-scoped, thread-safe)
        request_token.set(has_header)
...
@app.tool()
async def protected(ctx: Context[ServerSession, None]) -> str:
    """Access protected data - requires Admin.Write scope."""
    token = request_token.get()
       
    if not has_scope(token, "Admin.Write"):
        return "Forbidden - insufficient scopes"
    
    # Success case - user has valid token with correct scope
    return "Success: This is protected data that only users with the Admin.Write scope can access."
